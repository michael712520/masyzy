...
if (!$smarty->is_cached('categorycx.dwt', $cache_id))
{
    /* 如果页面没有被缓存则重新获取页面的内容 */
  /* links */
...
//        exit;
    ...
    if ($cat['grade'] > 1||$cat_id==0)
    {
        ...
        */
        ...
        $sql = "SELECT min(g.shop_price) AS min, max(g.shop_price) as max ".
               ...
               " WHERE ($children OR " . get_extension_goods($children) . ') AND g.is_delete = 0  and promote_price <> 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1  ';
               //获得当前分类下商品价格的最大值、最小值
        ...
        $sql = "SELECT (FLOOR((g.shop_price - $row[min]) / $dx)) AS sn, COUNT(*) AS goods_num  ".
               ...
               " WHERE ($children OR " . get_extension_goods($children) . ') AND g.is_delete = 0 and promote_price <> 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 '.
               " GROUP BY sn ";
       // $smarty->assign('sqlsadasdasdas',$sql);    
        ...
        foreach ($price_grade as $key=>$val)
        {
            ...
            $price_grade[$temp_key]['url'] = build_uricx('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_grade[$temp_key]['start'], 'price_max'=> $price_grade[$temp_key]['end'], 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
        ...
        $price_grade[0]['url'] = build_uricx('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>0, 'price_max'=> 0, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
    ...
    /* 品牌筛选 */
     ...
     $sql = "SELECT b.brand_id, b.brand_name, COUNT(*) AS goods_num ".
            ...
            " and promote_price <> 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".
    ...
    foreach ($brands AS $key => $val)
    {
        ...
        $brands[$temp_key]['url'] = build_uricx('category', array('cid' =>$cat_id, 'bid' => $val['brand_id'], 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str), "全部分类");
    ...
    $brands[0]['url'] = build_uricx('category', array('cid' => $cat_id, 'bid' => 0, 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
        ...
        foreach ($cat_filter_attr AS $key => $value)
        {
            ...
            if($temp_name = $db->getOne($sql))
            {
                ...
                $sql = "SELECT a.attr_id, MIN(a.goods_attr_id ) AS goods_id, a.attr_value AS attr_value FROM " . $ecs->table('goods_attr') . " AS a, " . $ecs->table('goods') .
                       " AS g" .
                       " WHERE ($children OR " . get_extension_goods($children) . ') and promote_price <> 0 AND g.goods_id = a.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 '.
                       " AND a.attr_id='$value' ".
    ...
    assign_pagerall('category',            $cat_id, $count, $size, $sort, $order, $page, '', $brand, $price_min, $price_max, $display, $filter_attr_str); // 分页
    assign_dynamic('category'); // 动态内容
}

$smarty->display('categorycx.dwt', $cache_id);
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $where = "g.is_on_sale = 1 and promote_price <> 0 AND g.is_alone_sale = 1 AND ".
...
{
    $where  = " g.is_on_sale = 1 and promote_price <> 0 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND ($children OR " . get_extension_goods($children) . ')';
