...
function get_child_treeppt($tree_id = 0)
{
    $three_arr = array();
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$tree_id' AND is_show = 1 ";
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
               ...
               $three_arr[$row['cat_id']]['id']   = $row['cat_id'];
               $three_arr[$row['cat_id']]['name'] = $row['cat_name'];
               $three_arr[$row['cat_id']]['url']  = build_urippt('category', array('cid' => $row['cat_id']), $row['cat_name']);

               if (isset($row['cat_id']) != NULL)
                   {
                       $three_arr[$row['cat_id']]['cat_id'] = get_child_treeppt($row['cat_id']);
 ...
 * @return  array
 */
function get_categories_treeActiveppt($cat_id = 0,$Active=0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                $cat_arr[$row['cat_id']]['id']   = $row['cat_id'];
                $cat_arr[$row['cat_id']]['name'] = $row['cat_name'];
                $cat_arr[$row['cat_id']]['url']  = build_urippt('category', array('cid' => $row['cat_id']), $row['cat_name']);
				$cat_arr[$row['cat_id']]['selected']=0;
                if($row['cat_id']==$Active){
                	$cat_arr[$row['cat_id']]['selected']=1;
                }
                if (isset($row['cat_id']) != NULL)
                {
                    $cat_arr[$row['cat_id']]['cat_id'] = get_child_treeppt($row['cat_id']);
                }
            }
        }
    }
 ...
 * @return  array
 */
function get_categories_treeActive($cat_id = 0,$Active=0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $goods = array();
    $childrencat=array();
    $sqlcategory='select * from '. $GLOBALS['ecs']->table('category').' as c where c.parent_id='.$cat_id.' order by sort_order limit 10';
    $rescategory = $GLOBALS['db']->getAll($sqlcategory);
    foreach ($rescategory as $val){
    	$childrencat[$val['cat_id']]['cat_name']=$val['cat_name'];
    	$childrencat[$val['cat_id']]['cat_id']=$val['cat_id'];
    }
    
    //$goods['children']=$childrencat;
    
    $cat['children'] = $childrencat;
    $sqlchild = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
    		"IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
    		'g.promote_price, promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img ' .
    		"FROM " . $GLOBALS['ecs']->table('goods') . ' AS g '.
    		"LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
    		"ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
    		'WHERE g.is_best and g.is_on_sale = 1 AND g.is_alone_sale = 1 AND '.
    		'g.is_delete = 0 AND (' . $children . 'OR ' . get_extension_goods($children) . ') ';
    
    $order_rulechild = empty($order_rule) ? 'ORDER BY g.sort_order, g.goods_id DESC' : $order_rule;
    $sqlchild .= $order_rulechild;
    if ($num > 0)
    {
    	$sqlchild .= ' LIMIT 1' ;
    }
    
    $reschild = $GLOBALS['db']->getAll($sqlchild);
    $isbest =array();
   foreach ($reschild as $val){
   	$isbest[$val['cat_id']]['goods_thumb']=$val['goods_thumb'];
   	
   	$isbest[$val['cat_id']]['url']=build_uri('goods', array('gid' => $val['goods_id']), $val['goods_name']);
   }
   //$goods['isbestgood']=$isbest;
   $cat['isbestgood'] = $isbest;
