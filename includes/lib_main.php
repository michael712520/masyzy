 ...
 * @return  void
 */
function assign_pagerallppt($app, $cat, $record_count, $size, $sort, $order, $page = 1,
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='')
{
    ...
    if ($pager['styleid'] == 0)
    {
        ...
        {
            $pager['page_first'] = build_uriallppt($app, $uri_args, '', 1, $keywords);
            $pager['page_prev']  = build_uriallppt($app, $uri_args, '', $page_prev, $keywords);
            $pager['page_next']  = build_uriallppt($app, $uri_args, '', $page_next, $keywords);
            $pager['page_last']  = build_uriallppt($app, $uri_args, '', $page_count, $keywords);
        }
        $pager['array']      = array();
    ...
    else
    {
        ...
        {
            $pager['page_first'] = ($page - $_offset > 1 && $_pagenum < $page_count) ? build_uriallppt($app, $uri_args, '', 1, $keywords) : '';
            $pager['page_prev']  = ($page > 1) ? build_uriallppt($app, $uri_args, '', $page_prev, $keywords) : '';
            $pager['page_next']  = ($page < $page_count) ? build_uriallppt($app, $uri_args, '', $page_next, $keywords) : '';
            $pager['page_last']  = ($_to < $page_count) ? build_uriallppt($app, $uri_args, '', $page_count, $keywords) : '';
            $pager['page_kbd']  = ($_pagenum < $page_count) ? true : false;
            $pager['page_number'] = array();
            for ($i=$_from;$i<=$_to;++$i)
            {
                $pager['page_number'][$i] = build_uriallppt($app, $uri_args, '', $i, $keywords);
            }
        }
    }
    if (!empty($sch_array))
 ...
 * @return  void
 */
function assign_pagerall($app, $cat, $record_count, $size, $sort, $order, $page = 1,
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='')
{
    $sch = array('keywords'  => $keywords,
                 'sort'      => $sort,
    ...
    if ($pager['styleid'] == 0)
    {
        ...
        {
            $pager['page_first'] = build_uriall($app, $uri_args, '', 1, $keywords);
            $pager['page_prev']  = build_uriall($app, $uri_args, '', $page_prev, $keywords);
            $pager['page_next']  = build_uriall($app, $uri_args, '', $page_next, $keywords);
            $pager['page_last']  = build_uriall($app, $uri_args, '', $page_count, $keywords);
        }
        $pager['array']      = array();
    ...
    else
    {
        ...
        {
            $pager['page_first'] = ($page - $_offset > 1 && $_pagenum < $page_count) ? build_uriall($app, $uri_args, '', 1, $keywords) : '';
            $pager['page_prev']  = ($page > 1) ? build_uriall($app, $uri_args, '', $page_prev, $keywords) : '';
            $pager['page_next']  = ($page < $page_count) ? build_uriall($app, $uri_args, '', $page_next, $keywords) : '';
            $pager['page_last']  = ($_to < $page_count) ? build_uriall($app, $uri_args, '', $page_count, $keywords) : '';
            $pager['page_kbd']  = ($_pagenum < $page_count) ? true : false;
            $pager['page_number'] = array();
            for ($i=$_from;$i<=$_to;++$i)
            {
                $pager['page_number'][$i] = build_uriall($app, $uri_args, '', $i, $keywords);
            }
        }
    }
    if (!empty($sch_array))
 ...
 * @return  void
 */
function show_message_wx($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
    assign_template();
    ...
    $GLOBALS['smarty']->assign('auto_redirect', $auto_redirect);
    $GLOBALS['smarty']->assign('message', $msg);
    $GLOBALS['smarty']->display('message_wx.dwt');
