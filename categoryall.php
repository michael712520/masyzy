...
if (!$smarty->is_cached('categoryall.dwt', $cache_id))
{
    /* 如果页面没有被缓存则重新获取页面的内容 */
...
//        exit;
   ...
   /* links */
    ...
    if ($cat['grade'] > 1||$cat_id==0)
    {
       ...
       // $smarty->assign('sqlsadasdasdas',$sql);    
        ...
        foreach ($price_grade as $key=>$val)
        {
            ...
            $price_grade[$temp_key]['url'] = build_uriall('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_grade[$temp_key]['start'], 'price_max'=> $price_grade[$temp_key]['end'], 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
        ...
        $price_grade[0]['url'] = build_uriall('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>0, 'price_max'=> 0, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
    ...
    foreach ($brands AS $key => $val)
    {
        ...
        $brands[$temp_key]['url'] = build_uriall('category', array('cid' =>$cat_id, 'bid' => $val['brand_id'], 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str), "全部分类");
    ...
    $brands[0]['url'] = build_uriall('category', array('cid' => $cat_id, 'bid' => 0, 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str), $cat['cat_name']);
...
$smarty->display('categoryall.dwt', $cache_id);
